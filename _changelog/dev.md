...
- GP-3028: 全局游戏设置中 `版本隔离` 下方不再展示路径
- GP-3023: `自动安装` 中已安装的加载器如由其他启动器安装，则不再允许移除/修改版本

错误修复:

- GP-3081: 启动器进程终止时停止日志线程，修复退出时卡死的问题
- GP-3082: 修复 NeoForge 整合包版本识别不正确的问题
- GP-3023: 修复 1.20.4 Forge 和 OptiFine 部分版本同时安装的情况下，游戏无法启动的问题
- GP-3027: 使 `resources` 目录跟随游戏目录，修复启用 `版本隔离` 后部分游戏版本没有声音的问题
- GP-3089: 修复模组下载界面版本推荐功能，优先根据加载器进行推荐
