...
- 游戏非正常退出时，请点击左下角 <a class="btn btn--primary btn--small">导出游戏崩溃日志</a> 按钮（不是 <a class="btn btn--primary btn--small">日志</a> 按钮），并完整发送生成的报错压缩包
...
<!----{{'>'}}
{% capture crash-support-group-info %}
<!---->
...
<!----{{'>'}}
{% endcapture %}
<div id="crash-support-group-info">{{ crash-support-group-info | markdownify }}</div>
<div id="crash-support-group-notice" class="notice--success">
<p class="text-center">请认真阅读注意事项，等待 <span id="countdown">5</span> 秒后可查看入群方式</p>
...
document.addEventListener("DOMContentLoaded", function () {
  var info = document.getElementById("crash-support-group-info");
  var notice = document.getElementById("crash-support-group-notice");
  ...
  var timer = setInterval(function () {
    ...
    if (sec <= 0) {
      clearInterval(timer);
      text.parentNode.textContent = "请点击下方按钮查看入群方式";
      btn.disabled = false;
    }
  }, 1000);
